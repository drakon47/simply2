...
try {
    ...
    // Eliminar el registro de la base de datos (DELETE físico)
    $stmt = $db->prepare("DELETE FROM documentos_centro_adicional WHERE id = ? AND centro_id = ?");
    $stmt->execute([$documento_id, $centro_id]);
    
    // Redirigir con mensaje de éxito
    header('Location: documentos_centro.php?success=1&mensaje=' . urlencode('Documento adicional eliminado exitosamente'));
    exit();
