...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_FILES['archivo'])) {
    try {
        $docu_requerida_centro_id = $_POST['docu_requerida_centro_id'] ?? null;
        ...
        if (!$docu_requerida_centro_id) {
            throw new Exception('ID de documento requerido no válido');
        }
        
        $stmt = $db->prepare("SELECT id FROM docu_requerida_centro WHERE id = ? AND activo = 1");
        $stmt->execute([$docu_requerida_centro_id]);
        if (!$stmt->fetch()) {
        ...
        $stmt = $db->prepare("SELECT COALESCE(MAX(id), 0) + 1 as siguiente_id FROM documentos_centro_subidos WHERE centro_id = ?");
        ...
        // Generar nombre de archivo codificado: centro_id_docu_id_documento_id.extension
        $nombre_codificado = $centro_id . '_' . $docu_requerida_centro_id . '_' . $documento_id . '.' . $extension;
        ...
        $stmt = $db->prepare("
            INSERT INTO documentos_centro_subidos (
                docu_requerida_centro_id, 
                centro_id, 
                nombre_archivo, 
                ruta_archivo, 
                fecha_vencimiento, 
        ...
        header('Location: documentos_centro.php?success=1');
        exit();
